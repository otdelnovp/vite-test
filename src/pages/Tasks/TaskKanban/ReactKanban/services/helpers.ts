...
function moveColumn(board: any, { fromPosition }: { fromPosition: any }, { toPosition }: { toPosition: any }) {
  return { ...board, columns: changeElementOfPositionInArray(board.columns, fromPosition, toPosition) }
}

function moveCard(board: any, { fromPosition, fromColumnId }: { fromPosition: any, fromColumnId: any }, { toPosition, toColumnId }: { toPosition: any, toColumnId: any }) {
...
function removeColumn(board: any, column: any) {
  return { ...board, columns: board.columns.filter(({ id }: { id: any }) => id !== column.id) }
}

function changeColumn(board: any, column: any, newColumn: any) {
  const changedColumns = replaceElementOfArray(board.columns)({
    when: ({ id }: { id: any }) => id === column.id,
    for: (value: any) => ({ ...value, ...newColumn }),
  })
  return { ...board, columns: changedColumns }
}

function addCard(board: any, inColumn: any, card: any, { on }: { on: string }) {
  const columnToAdd = board.columns.find(({ id }: { id: any }) => id === inColumn.id)
...
function removeCard(board: any, fromColumn: any, card: any) {
  const columnToRemove = board.columns.find(({ id }: { id: any }) => id === fromColumn.id)
  const filteredCards = columnToRemove.cards.filter(({ id }: { id: any }) => card.id !== id)
...
function changeCard(board: any, cardId: any, newCard: any) {
  const changedCards = (cards: any) =>
    replaceElementOfArray(cards)({
      when: ({ id }: { id: any }) => id === cardId,
      for: (card: any) => ({ ...card, ...newCard }),
    })

  return { ...board, columns: board.columns.map((column: any) => ({ ...column, cards: changedCards(column.cards) })) }
}
